...
public final class AsmEncoder {
    ...
    public static int encode(String mnemonic, int pc, List<Token> args, Map<String,Integer> labelAddrs) {
        ...
        if (CALL.equals(mnemonic))  return encodeCALL(pc, args, labelAddrs);
        if (RET.equals(mnemonic))   return encodeRET();

        InstructionDef def = InstructionTable.DEF.get(mnemonic);
        if (def == null) throw new IllegalArgumentException("Unknown instruction: " + mnemonic);
    ...
    private static int regNum(Token t) {
        if (t.getType() != TokenType.REGISTER) throw new IllegalArgumentException("Expected register: " + t);
        String v = t.getValue().trim().toLowerCase();
        if (!v.startsWith("x")) throw new IllegalArgumentException("Register must start with 'x': " + v);
    ...
    private static int immVal(Token t) {
        if (t.getType() != TokenType.IMMEDIATE) throw new IllegalArgumentException("Expected immediate: " + t);
        String s = t.getValue().trim();
        // Integer.decode handles 0x.., 0..(octal), decimal and signed variants
    ...
    private static int labelAddr(Token t, Map<String,Integer> labelAddrs) {
        if (t.getType() != TokenType.LABEL) throw new IllegalArgumentException("Expected label: " + t);
        Integer addr = labelAddrs.get(t.getValue());
        if (addr == null) throw new IllegalArgumentException("Undeclared label: " + t.getValue());
        return addr;
    }
    ...
    private static int encodeR(InstructionDef def, List<Token> args) {
        if (args.size() != 3) throw new IllegalArgumentException("R-type expects 3 args");
    ...
    /** I-type: (addi/andi/ori) rd, rs1, imm  | (lw) rd, imm(rs1)  | (jalr) rd, rs1, imm */
    private static int encodeI(String mnemonic, InstructionDef def, int pc, List<Token> args, Map<String,Integer> labels) {
        if (ADDI.equals(mnemonic) || ANDI.equals(mnemonic) || ORI.equals(mnemonic) || "jalr".equals(mnemonic)) {
            if (args.size() != 3) throw new IllegalArgumentException(mnemonic + " expects 3 args");
            int rd  = regNum(args.get(0));
            int rs1 = regNum(args.get(1));
        ...
        if ("lw".equals(mnemonic)) {
            ...
            if (args.size() != 3) throw new IllegalArgumentException("lw expects: rd, imm, baseReg");
            int rd  = regNum(args.get(0));
        ...
        throw new IllegalArgumentException("Unsupported I-type mnemonic: " + mnemonic);
    }

    /** S-type: sw rs2, imm(rs1)  -> logical args: rs2, imm, rs1 */
    private static int encodeS(InstructionDef def, List<Token> args) {
        if (args.size() != 3) throw new IllegalArgumentException("sw expects: rs2, imm, baseReg");
    ...
    private static int encodeB(InstructionDef def, int pc, List<Token> args, Map<String,Integer> labels) {
        if (args.size() != 3) throw new IllegalArgumentException("beq expects: rs1, rs2, label");
        int rs1 = regNum(args.get(0));
        int rs2 = regNum(args.get(1));
        int target = labelAddr(args.get(2), labels);
        int off = target - pc;
        if ((off & 0x1) != 0) throw new IllegalArgumentException("Branch offset must be 2-byte aligned");
        // 12-bit signed immediate (in bytes). We check range in bytes:
        if (off < -4096 || off > 4094) throw new IllegalArgumentException("Branch offset out of range: " + off);
    ...
    private static int encodeU(InstructionDef def, List<Token> args) {
        if (args.size() != 2) throw new IllegalArgumentException("lui expects: rd, imm20");
        int rd  = regNum(args.get(0));
    ...
    private static int encodeJ(InstructionDef def, int pc, List<Token> args, Map<String,Integer> labels) {
        ...
        off = target - pc; 
        if ((off & 0x1) != 0) throw new IllegalArgumentException("Jump offset must be 2-byte aligned");
        if (off < -(1<<20) || off > ((1<<20)-2)) throw new IllegalArgumentException("Jump offset out of range: " + off);
    ...
    private static int encodeCALL(int pc, List<Token> args, Map<String,Integer> labels) {
        if (args.size() != 1 || args.get(0).getType() != TokenType.LABEL)
            throw new IllegalArgumentException("call expects one label");
        InstructionDef jal = InstructionTable.DEF.get(JAL);
        int rd = 1; // ra = x1
        int target = labelAddr(args.get(0), labels);
        int off = target - pc;
        if ((off & 1) != 0) throw new IllegalArgumentException("Jump offset must be 2-byte aligned");
        if (off < -(1<<20) || off > ((1<<20)-2)) throw new IllegalArgumentException("Jump offset out of range: " + off);
    ...
    private static int imm12(int v) {
        if (v < -2048 || v > 2047) throw new IllegalArgumentException("12-bit immediate out of range: " + v);
        return v & 0xFFF;
    }

    /** Clamp to 20-bit unsigned immediate (U-type) */
    private static int imm20(int v) {
        if (v < 0 || v > 0xFFFFF) throw new IllegalArgumentException("20-bit immediate out of range: " + v);
        return v & 0xFFFFF;
    }
